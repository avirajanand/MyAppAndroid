...
/**
 * Activity that starts PlayTangLoginActivity if the user is not logged in.
 * Otherwise, it starts the subclass-defined target activity.
 * 
 * To use this, you should subclass this activity and implement
 * {@link PlayTangLoginDispatchActivity#getTargetClass} to return the class of the
 ...
 */
public abstract class PlayTangLoginDispatchActivity extends Activity {
  ...
  private static final String LOG_TAG = "PlayTangLoginDispatch";
  ...
  /**
   * Override this to generate a customized intent for starting PlayTangLoginActivity.
   ...
   *
   * @return Intent that can be used to start PlayTangLoginActivity
   */
  protected Intent getPlayTangLoginIntent() {
    PlayTangLoginBuilder builder = new PlayTangLoginBuilder(this);
    return builder.build();
  }

  private void runDispatch() {
    if (ParseUser.getCurrentUser() != null) {
      debugLog(getString(com.playtang.commonnavigation.R.string.com_parse_ui_login_dispatch_user_logged_in) + getTargetClass());
      startActivityForResult(new Intent(this, getTargetClass()), TARGET_REQUEST);
    } else {
      debugLog(getString(com.playtang.commonnavigation.R.string.com_parse_ui_login_dispatch_user_not_logged_in));
      startActivityForResult(getPlayTangLoginIntent(), LOGIN_REQUEST);
    }
  }
